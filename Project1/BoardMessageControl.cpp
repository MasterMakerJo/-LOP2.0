...
void BoardMessageControl::InputMsg(struct Ddz * pDdz)
{
	...
	if(pDdz->iRoundNow>1)
	cout << endl<<"				总共进行了 " << pDdz->iRoundNow-1 << " 局, AI赢了 " << setiosflags(ios::fixed) << setprecision(0)<<pDdz->dWinCount << " 局， 胜率为：" << setiosflags(ios::fixed) <<setprecision(2)<< 100*((double)(pDdz->dWinCount) / (pDdz->iRoundNow-1)) << "%" << endl;
	...
	cout <<endl<< " 开始斗地主 （输入出牌请用空格隔开,只输入0为PASS,J Q K A 小王 大王分别为11 12 13 1 14 15,最后用0结束）" << endl<<endl;
 {
		...
		int us[21] = { 46, 10, 17, 42, 51, 14, 30, 37, 4, 43, 44, 20, 35, 47, 50, 21, 29 ,-1,-1,-1,-1};//我们的牌
		int latter[21] = { 33, 36, 22, 26, 9, 48, 27, 52, 2, 13, 12, 39, 19, 24, 0, 16, 8,-1,-1,-1,-1 };//下家的牌
		int former[21] = { 3, 7, 45, 32, 49, 6, 15, 28, 5, 11, 34, 25, 41, 18, 40, 38, 53 ,-1,-1,-1,-1 };//上家的牌
		out[51] = 23;
		out[52] = 1;
		out[53] = 31; 


			/*A3, 7, 45, 32, 49, 6, 15, 28, 5, 11, 34, 25, 41, 18, 40, 38, 53
			OK DEAL
			DEAL B46, 10, 17, 42, 51, 14, 30, 37, 4, 43, 44, 20, 35, 47, 50, 21, 29
			OK DEAL
			DEAL C33, 36, 22, 26, 9, 48, 27, 52, 2, 13, 12, 39, 19, 24, 0, 16, 8
			 A23,1,31
			*/
...
void BoardMessageControl::GetGam(struct Ddz * pDdz)
{
	...
	{
		if (pDdz->cLandlord != pDdz->cWinner)
		{
			pDdz->dWinCount++;
		}
	}
